...
public class ImageRepository {
    ...
    public Image getImageByTitle(String title, Timestamp date) {
        EntityManager em = emf.createEntityManager();
        try {
            TypedQuery<Image> typedQuery = em.createQuery("SELECT i from Image i where i.title =:title and i.date =:date", Image.class).setParameter("title", title).setParameter("date", date);
    ...
    public Image getImage(Integer imageId) {
        EntityManager em = emf.createEntityManager();
        String sql = "select i from images i where i.id=:imageId and  i.user_id in(select id from users u where u.id=i.user_id )";
        Query q = em.createNativeQuery(sql).setParameter("imageId",imageId);
        List<Image> imageResult =  q.getResultList();
        Image image = (Image) imageResult.get(0);
